 ...
 * @author Ian
 */
public class ShiftedPoly_Solver {

   public void ShiftedPoly_Solver() {

   }

   public void Solve_ShiftedPoly(JTable B2_table, JTable table) {

      // Start on first line of Base2_table and convert
      ...
      row_ptr = 0;
      do {
         table.setValueAt(current_poly_str, row_ptr, 0);
         row_ptr++;
      } while (row_ptr < table.getRowCount());


      // scan first line to establish Max_X power
      max_line_length = B2_table.getColumnCount();
      Max_X_Power = 0;
      pos_count = 0;
      while ((Max_X_Power == 0) && (pos_count < max_line_length)) {
         if (B2_table.getValueAt(0, pos_count) != "") {
            pos_count++;
         } else {
      ...
      do { // do conversions
         ...
         while (current_pos < pos_count) {
            if (B2_table.getValueAt(row_ptr, current_pos) == "1") {
               ...
               if ((B2_table.getValueAt(row_ptr, current_pos) == "0") && !line_started){
         ...
         if (!lineStartsWithZero) {
            table.setValueAt((indent_string + current_poly_str), row_ptr, 0);
         } else {
            current_poly_str = "";
